...
def request(employee="E001",start=date(2027,1,10),days=2): return LeaveRequestCreate(employee_id=employee,start_date=start,end_date=date.fromordinal(start.toordinal()+days-1),days=days,reason="Family event")
def test_approved_request(service):
 created=service.submit(request()); decided=service.decide(created["id"],LeaveDecision(manager_id="M001",decision="APPROVED")); assert decided["status"]=="APPROVED"; assert "AC-002" in decided["traceability"]; assert service.balance("E001")["available_days"]==10
...
def test_date_range_is_authoritative_for_requested_days(service):
 misleading=LeaveRequestCreate(employee_id="E002",start_date=date(2027,2,1),end_date=date(2027,2,5),days=1,reason="Family event")
 with pytest.raises(LeaveDomainError) as error: service.submit(misleading)
 assert error.value.code=="INSUFFICIENT_BALANCE"
