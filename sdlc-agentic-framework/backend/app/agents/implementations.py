...
class BRDAgent:
    def run(self, req, context):
        ...
        rules='; '.join(x['text'] for x in req['business_rules'])
        sections={'Executive Summary':f"Automate {feature} with governed, traceable workflow outcomes.",'Business Objective':f"Reduce manual handoffs and improve the reliability of {feature}.",'Scope':req['raw_text'],'Out of Scope':'Capabilities not stated in the submitted requirement and production identity integration.','Functional Requirements':f"FR-001 capture and validate {feature} inputs; FR-002 apply business rules ({rules}); FR-003 record workflow outcomes and audit events.",'Non-Functional Requirements':'; '.join(x['text'] for x in req['non_functional_requirements']),'Assumptions':'; '.join(x['text'] for x in req['assumptions']),'Constraints':'; '.join(req['constraints']),'Risks':'; '.join(context.get('known_risk_areas',[])) or 'Incomplete domain rules and integration availability.','Dependencies':'; '.join(context.get('related_code_modules',[])) or 'Domain data sources and identity services.','Acceptance Criteria Summary':f"Valid {feature} inputs are processed according to the stated rules; invalid inputs are rejected with clear errors."}
        return f'# BRD-001 — {feature}\n\n**Traceability:** REQ-001\n\n'+''.join(f'## {k} (confidence: 0.90)\n{v}\n\n' for k,v in sections.items())
class StoryAgent:
    def run(self, brd):
        heading=re.search(r'^# BRD-001 — (.+)$',brd,re.MULTILINE)
        feature=heading.group(1) if heading else 'Employee Leave Management System'
        if feature == 'Employee Leave Management System':
            ...
            specs=[('STORY-001',f'Submit {feature} input','User','submit valid input','input is provided','the input is validated and recorded'),('STORY-002',f'Process {feature} workflow','User','process the workflow','validated input exists','business rules are applied and an outcome is recorded'),('STORY-003',f'View {feature} status','User','view the current status','a workflow record exists','its current status and audit history are shown')]
        stories=[]
        for i,(sid,title,persona,when,given,then) in enumerate(specs,1): stories.append({'id':sid,'title':title,'persona':persona,'narrative':f'As a {persona}, I want to {title.lower()} so that {feature.lower()} is managed.','story_points':[5,3,2][i-1],'priority':'High','dependencies':[] if i==1 else ['STORY-001'],'subtasks':['API and validation','UI and tests'],'acceptance_criteria':[{'id':f'AC-{i:03d}','given':given,'when':when,'then':then,'requirement_id':'REQ-001'}]})
        return {'requirement_id':'REQ-001','epics':[{'id':'EPIC-001','title':f'{feature} lifecycle','stories':stories}]}
